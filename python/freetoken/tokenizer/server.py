...
def _tokenize_requests(
    ...
    logger: Any,
) -> tuple[List[TokenizeMsg], List[torch.Tensor], List[dict | None], List[UserReply]]:
    ...
    ok_mm: List[dict | None] = []
    errors: List[UserReply] = []
    for msg in messages:
        try:
            tokens, mm = tokenize_manager.tokenize_with_mm([msg])[0]
        ...
        ok_mm.append(mm)
    return ok_msgs, ok_tensors, ok_mm, errors
    ...
    try:
        while True:
            ...
            if len(tokenize_msg) > 0:
                ...
                ok_msgs, ok_tensors, ok_mm, errors = _tokenize_requests(
